...
SET SERVEROUTPUT ON;
SET PAGES 0;
SPOOL 'colonne_up_final4.sql'

    -- largeur de colonne
    SELECT 'UPDATE audit.colonne SET largeurColonne=',data_length,
	'WHERE nom =trim( ''', column_name ,''' )AND nomTable =trim( ''', table_name ,''' ) AND owner=trim(''', owner,
	''');'
